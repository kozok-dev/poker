...
function getProfile() {
	return {
		image: images["player4-1.jpg"].src,
		description: "ビリヤードが好き。"
	};
}
...
class ComPlayer {
	getImage(player) {
		if (player.action == Player.ACTION.BANKRUPT) return images["player4-4.jpg"].src;
		if (player.result == Player.RESULT.WIN) return images[player.betTotal < 4 ? "player4-3.jpg" : "player4-2.jpg"].src;
		if (player.result == Player.RESULT.LOSE) {
			return images[player.betTotal < 2 && player.chip > Math.floor(store.initChip / 2) ? "player4-1.jpg" : "player4-4.jpg"].src;
		}
		if (player.action == Player.ACTION.FOLD) {
			return images[player.betTotal < 4 && player.chip > Math.floor(store.initChip / 5) ? "player4-1.jpg" : "player4-4.jpg"].src;
		}
		return images["player4-1.jpg"].src;
	}
	...
	getBet(player, players, betMax) {
		...
		case Player.HAND.ONE_PAIR:
			if (player.hand.details[0].number <= 10) {
				foldRate *= 0.5 - 0.45 * ((player.hand.details[0].number - 2) / 8);
			} else {
				foldRate = 0;
			}
			break;
		...
		if (Math.random() < foldRate && Math.random() < 0.8) return -1;
		...
		case Player.HAND.TWO_PAIR:
			betRate = Math.random() < 0.5 ? 0.5 : 0.3;
			break;
		default:
			betRate = Math.random() < 0.5 ? 0.7 : 0.3;
			break;
		}

		let bet = 0;
		for (let i = 0; i < 5; i++) {
			if (Math.random() < betRate) bet++;
		}
	...
	getChangeCards(cards) {
		...
		if (!isHold) {
			for (let card of cards) {
				if (card.number == 11 && Math.random() < 0.3) card.isHold = true;
				if (card.number == 12 && Math.random() < 0.4) card.isHold = true;
				if (card.number == 13 && Math.random() < 0.6) card.isHold = true;
